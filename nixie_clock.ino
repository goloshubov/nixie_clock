...
constexpr uint8_t DND_BEGIN_HOUR = 23;
constexpr uint8_t DND_END_HOUR = 7;

// Brightness settings default / DND mode
...
void setup() {
    ...
    clock.begin();
    read_time();
    clock.set(current_hour, current_minute, 0, 3, 1, 2024, 1);
...
void set_brightness() {
  ...
  if (((DND_BEGIN_HOUR == DND_END_HOUR) and (DND_BEGIN_HOUR == current_hour)) or                                      // 23 - 23
    ...
    dot.setBrightness(DOT_BRIGHTNESS_DND);
  } else {
    // Default
